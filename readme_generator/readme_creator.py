...
class ReadmeCreator:
    ...
    def __init__(self):
        ...
        self.final_readme_chain = self.config.final_readme_chain
    ...
    def cluster_and_select_representatives(self, documents: list[str], max_clusters: int = 5) -> list[str]:
        logging.info(f"Clustering {len(documents)} documents into up to {max_clusters} clusters.")

        # 문서 임베딩 생성
        embeddings: list[np.ndarray] = [self.embeddings_model.embed_query(text=doc) for doc in documents]
        embeddings_matrix: np.ndarray = np.array(embeddings)

        # 클러스터 수를 문서 수에 맞게 조정
        num_clusters: int = min(max_clusters, len(documents))
    ...
    def create_readme_for_folder(self, combined_content: str, language: str = "kor") -> str | None:
        logging.info(f"Creating README for folder with content size: {len(combined_content)}")

        split_documents: list[str] = self.splitter.split_text(combined_content)

        # 청크 크기를 토큰 수로 제한
        valid_documents: list[str] = []
        for doc in split_documents:
            token_count: int = count_tokens(doc, model=self.config.openai_model)
        ...
        # 대표 문서 요약
        summarized_chunks: list[str] = self.code_summary_chain.batch([{"context": doc} for doc in representative_docs])

        # 요약된 내용을 병합하여 최종 README 생성
        summarized_text: str = "\n".join(summarized_chunks)
        readme_content: str | None = self.readme_chain.invoke({"language": language, "context": summarized_text})

        if not readme_content:
            logging.error("The model returned None for the README content.")
        return readme_content

    def create_final_readme(self, combined_summaries: str) -> str | None:
        """전체 프로젝트에 대한 최종 README를 생성합니다."""
        readme_content: str | None = self.final_readme_chain.invoke({"context": combined_summaries})
